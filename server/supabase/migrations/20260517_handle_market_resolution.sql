...
DECLARE
  ...
  payout NUMERIC;
BEGIN
  -- Collect all option IDs for this market
  SELECT ARRAY_AGG(id) INTO option_ids
  FROM market_options
  WHERE market_id = p_market_id;

  -- Pay out winning users
  FOR rec IN
    SELECT
      up.user_id,
      up.shares_owned,
    ...
    WHERE up.market_option_id = p_resolved_option_id
      AND up.shares_owned > 0
  LOOP
    ...
    WHERE user_id = rec.user_id
      AND market_option_id = rec.market_option_id;
  END LOOP;

  -- Close losing positions (no payout)
  FOR rec IN
    SELECT
      up.user_id,
      up.shares_owned,
      ...
      up.market_option_id
    FROM user_positions up
    WHERE up.market_option_id = ANY(option_ids)
      AND up.market_option_id <> p_resolved_option_id
      AND up.shares_owned > 0
  LOOP
    INSERT INTO transactions (user_id, market_option_id, type, shares, price_at_time, amount)
